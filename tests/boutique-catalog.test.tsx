...
describe("BoutiqueCatalog et ProductCard", () => {
  it("affiche le filtre et le contenu des produits sur la page", () => {
    render(<BoutiqueCatalog products={products} />);
    ...
    expect(screen.getByText(`${cafe.price} €`)).toBeInTheDocument();
    ...
    expect(screen.getByText(`${chocolat.price} €`)).toBeInTheDocument();
    ...
    expect(screen.getByText(`${mate.price} €`)).toBeInTheDocument();
  });
});
